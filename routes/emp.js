    ...
    if(resultOfValidation.error==null)
{
    ...
    let query = `insert into mean_table values(${eno}, '${ename}', '${eddress}')`;
    console.log(query);
...
emprouter.put("/:No",function(request, response){
    ...
    let query = `update mean_table set name= '${ename}',address= '${eddress}' where no=${eno}`;
    console.log(query);
...
emprouter.delete("/:No",function(request, response){
    let eno = parseInt(request.params.No);
    let query = `delete from mean_table where no=${eno}`;
    console.log(query);
...
emprouter.get("/", function(request, response){
    connection.query("select * from mean_table", function(err, result){
        if(err==null)
        {
           myData =  result;
           response.contentType("application/json");
